...
#[test]
fn test_session_event_recovery_beats_nearby_bash_candidate() {
    let (_metrics_db_dir, metrics_db_path) = isolated_metrics_db_path();
    ...
    let commit = repo
        .stage_all_and_commit("Repo-linked session event beats bash noise")
        .expect("commit should succeed");
    ...
    assert!(
        commit
            ...
            .sessions
            .contains_key(&recovered_session_id),
        "repo-linked session event should recover attribution"
    );
    assert!(
        !commit
            .authorship_log
        ...
        "nearby bash candidate must not win over a repo-linked session event"
    );
}
